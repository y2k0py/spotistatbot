...
engine = create_engine('mysql://root:zekmBf3l9f4eKnAI2DHj@containers-us-west-125.railway.app:6369/railway')
...
class Users(Base):
    ...
    telegram_code = Column(String(10), unique=True, nullable=False)
    telegram_user_id = Column(String(15), unique=True)
...
async def on_start(message: types.Message):
    ...
    if link_code:
        target = session.query(Users).filter_by(telegram_code=link_code).first()
        if target:
            try:
                target.telegram_user_id = message.from_user.id
                session.commit()
        ...
        await message.answer('Hi, this is a bot to view your stats on Spotify. Follow this link to log in to Spotify: http://127.0.0.1:5000')
